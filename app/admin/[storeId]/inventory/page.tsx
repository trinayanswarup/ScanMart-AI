...
export default function AdminInventoryPage() {
  ...
  const { getStoreInventory, getStoreListings, updateInventory, publishItem } = useApp();
  ...
  return <div className="page-wrap">
    <div className="page-header"><div><h1>Inventory</h1><p>Manage product data, stock levels, and storefront visibility.</p></div><div style={{ display: "flex", gap: 10 }}><Link className="btn btn-secondary" href={`/admin/${storeId}/inventory/new`}><PackagePlus size={16} />Add manually</Link><Link className="btn btn-primary" href={`/admin/${storeId}/scan`}><Sparkles size={16} />Scan with AI</Link></div></div>
    <div className="card">
      <div style={{ padding: 16, borderBottom: "1px solid var(--line)", display: "flex", gap: 10, flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: "1 1 270px" }}><Search size={16} style={{ position: "absolute", left: 12, top: 13, color: "var(--muted)" }} /><input className="input" style={{ paddingLeft: 37 }} placeholder="Search products or brands..." value={search} onChange={(e) => setSearch(e.target.value)} /></div>
      ...
      {items.length ? <div className="table-wrap"><table className="table"><thead><tr><th>Product</th><th>Category</th><th>Stock</th><th>Price</th><th>Source</th><th>Storefront</th><th>Actions</th></tr></thead><tbody>{items.map((item) => {
        ...
        return <tr key={item.id}><td><Link href={`/admin/${storeId}/products/${item.id}`}><strong>{item.name}</strong><span className="muted" style={{ display: "block", fontSize: 11, marginTop: 4 }}>{item.brand || "Unbranded"}</span></Link></td><td>{item.category}</td><td><strong style={{ color: low ? "var(--accent)" : undefined }}>{item.quantity} {item.unit}</strong>{low && <span className="badge badge-amber" style={{ marginLeft: 8 }}>Low</span>}</td><td>{item.price ? `€${item.price}` : <span className="muted">Not set</span>}</td><td><span className={`badge ${item.source === "ai_scan" ? "badge-green" : "badge-gray"}`}>{item.source.replace("_", " ")}</span></td><td><span className={`badge ${listing?.isPublished ? "badge-green" : "badge-gray"}`}>{listing?.isPublished ? "Published" : "Unpublished"}</span></td><td><div style={{ display: "flex", gap: 5 }}><Link title="Open product" href={`/admin/${storeId}/products/${item.id}`} className="btn btn-ghost" style={{ padding: "0 8px" }}><ExternalLink size={15} /></Link>{!listing?.isPublished && <button title="Publish" className="btn btn-ghost" style={{ padding: "0 8px" }} onClick={() => item.price ? publishItem(item.id) : alert("Set a price before publishing.")}><Send size={15} /></button>}<button title="Archive" className="btn btn-ghost" style={{ padding: "0 8px" }} onClick={() => updateInventory(item.id, { status: "archived" })}><Archive size={15} /></button></div></td></tr>;
      })}</tbody></table></div> : <div className="empty"><div className="empty-icon"><Box /></div><h3>No products found</h3><p className="muted">Try clearing your filters or add your first product.</p><Link href={`/admin/${storeId}/scan`} className="btn btn-primary">Scan a product</Link></div>}
    </div>
