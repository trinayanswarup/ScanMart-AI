...
export default function InventoryPage() {
  const { state, updateInventory, publishItem } = useApp();
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState("all");
  const [lowOnly, setLowOnly] = useState(false);
  const categories = [...new Set(state.inventory.map((item) => item.category))];
  const items = useMemo(() => state.inventory.filter((item) =>
  ...
  ), [state.inventory, search, category, lowOnly]);
  ...
  return <div className="page-wrap">
    ...
    <div className="card">
      ...
      {items.length ? <div className="table-wrap"><table className="table"><thead><tr><th>Product</th><th>Category</th><th>Stock</th><th>Price</th><th>Source</th><th>Storefront</th><th>Actions</th></tr></thead><tbody>{items.map((item) => {
        const listing = state.listings.find((entry) => entry.inventoryItemId === item.id);
    ...
    <p className="muted" style={{ fontSize: 11, textAlign: "right" }}>Showing {items.length} of {state.inventory.filter((item) => item.status !== "archived").length} active items</p>
  </div>;
}
